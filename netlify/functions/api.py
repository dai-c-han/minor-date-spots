...
def build_overpass_query(lat, lon, radius, categories):
    if not categories:
...
def detect_category(tags):
    ...
    if tags.get("tourism") in ("gallery", "museum"): return "art"
    ...
    if tags.get("leisure") == "garden": return "garden"
    if tags.get("amenity") == "place_of_worship": return "shrine"
    return "other"
...
async def search_spots(req: SearchRequest):
    ...
    for spot in spots:
        spot["minor_score"] = max(0, 100 - len(spot["tags"]) * 5)
...
[
  {{
    ...
    try:
        ...
        suggestions = json.loads(text[text.find("["):text.rfind("]") + 1])
    except Exception:
